...
try {
  ...
  window.eval(`${appJs}
    window.__test = { state, rebuildTranscriptionIndex };
  `);
} catch (e) {
...
const { state, rebuildTranscriptionIndex } = window.__test;
...
setTimeout(() => {
  try {
    ...
    state.clips = [{ id: 'c1', kind: 'video', mediaId: 'vid1', start: 0, trimStart: 0, trimEnd: 10 }];
